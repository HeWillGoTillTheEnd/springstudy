...
public class NewsDAO {
    ...
    public Connection open() {
        ...
        try {
            ...
            conn = DriverManager.getConnection(JDBC_URL, "root", "fhftl1qkf!");
...
//    }
    ...
    public List<News> getAll() throws SQLException{
        ...
        pstmt = conn.prepareStatement("select aid, title, STR_TO_DATE(date, '%Y-%m-%d %h:%i:%s') as cdate from news");
    ...
    public News getNews(int aid) throws SQLException{
        ...
        pstmt = conn.prepareStatement("select aid, title, img, STR_TO_DATE(date, '%Y-%m-%d %h:%i:%s') as cdate, content from news where aid = ?");
        pstmt.setInt(1,aid);
        ResultSet rs = pstmt.executeQuery();
